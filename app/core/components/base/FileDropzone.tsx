...
const FileDropzone = ({ text, files, onChange, maxFiles, maxSize, warning }: IFileDropzone) => {
  ...
  const {
    ...
    onDrop: acceptedFiles => {
      ...
      onChange([...files, ...filteredFiles.map(file => Object.assign(file, {
        preview: URL.createObjectURL(file),
        id: file.lastModified.toString() + file.size.toString(),
      }))]);

      // setErrorFiles(duplicateFiles.map(file => Object.assign(file, {
      //   preview: URL.createObjectURL(file),
      //   id: file.lastModified.toString() + file.size.toString(),
      // })));
    },
  ...
  return (
    <div className="container">
      ...
      >
        ...
        <Stack py={64} justify="center" spacing={0} px="xl">
          <Group position="center" mb="lg">
            ...
            <Text weight="bold" size="lg">{text}</Text>
          </Group>
      ...
      {files.length ?
        <>
          <Text weight="bold" size="lg" mt="sm" mb="xs">Preview:</Text>
          <Group spacing="xs">
            {files.map((file, i) => (
              <Center
      ...
      {/* {errorFiles.length ?
      <Box>
        <Text weight="bold" color="red">These files are duplicates:</Text>
